...
export function makeSubPages(
  ...
  if (subpages) {
    // Bind Select buttons (1-3) to Mixer, Control Room, MIDI CC
    const selectPages = [subpages.mixer, subpages.controlRoom, subpages.midiCC];

    device.channelControls.forEach((channelControl, index) => {
      // Select buttons activate specific pages (cycling through first 3)
      if (index < selectPages.length && selectPages[index]) {
        const selectBinding = page
          .makeActionBinding(
            channelControl.buttons.select.mSurfaceValue,
            selectPages[index].mAction.mActivate
          )
          .setSubPage(subPage);
      }
